...
# Create a new client and connect to the server
    ...
    exclude_locations = ["world", "unknown","centre"]
    ...
    if filtered_df.empty:
        ...
        with col1:
            ...
            top_5_events = event_counts.groupby('disaster_event')['count'].sum().reset_index().sort_values(by='count', ascending=False).head(5)
        ...
        with col2:
            ...
            fig_vertical_bar = px.bar(
                top_10_countries,
                x='Country',  # Use 'Country' for x-axis
                y='Count',  # Use 'Count' for y-axis
                title='Top 10 Countries by Disaster Occurrences',
                labels={'Country': 'Country', 'Count': 'Count'},  # Custom labels for clarity
                width=800,
                height=500
            )
            ...
            fig_vertical_bar.update_layout(
                yaxis_tickformat='d'  # 'd' formats the ticks as integers
            )
